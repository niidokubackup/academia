...
router.get('/users', authorizeRoles('admin'), async (req, res) => {
  try {
    const users = await db.prepare('SELECT id, full_name, email, role, school, department, level, matric_number, created_at FROM users ORDER BY created_at DESC').all();
    res.json(users);
  } catch (err) {
...
router.get('/courses', authorizeRoles('admin'), async (req, res) => {
  try {
...
router.post('/courses', authorizeRoles('lecturer'), async (req, res) => {
  try {
...
router.put('/courses/:id/publish', authorizeRoles('admin'), async (req, res) => {
  try {
...
router.put('/courses/:id/unpublish', authorizeRoles('admin'), async (req, res) => {
  try {
    const courseId = Number(req.params.id);
    if (!Number.isInteger(courseId) || courseId <= 0) return res.status(400).json({ error: 'Invalid course id.' });
...
router.put('/courses/:id/delete', authorizeRoles('admin'), async (req, res) => {
  try {
    const courseId = Number(req.params.id);
    if (!Number.isInteger(courseId) || courseId <= 0) return res.status(400).json({ error: 'Invalid course id.' });
...
router.delete('/courses/:id', authorizeRoles('admin'), async (req, res) => {
  try {
    const courseId = Number(req.params.id);
    if (!Number.isInteger(courseId) || courseId <= 0) return res.status(400).json({ error: 'Invalid course id.' });
    await db.prepare('DELETE FROM courses WHERE id = ?').run(courseId);
...
// News approval
router.get('/news', authorizeRoles('admin'), async (req, res) => {
  try {
...
router.put('/news/:id/approve', authorizeRoles('admin'), async (req, res) => {
  try {
...
router.put('/news/:id/reject', authorizeRoles('admin'), async (req, res) => {
  try {
    const newsId = Number(req.params.id);
    if (!Number.isInteger(newsId) || newsId <= 0) return res.status(400).json({ error: 'Invalid news id.' });
...
router.get('/events', authorizeRoles('admin'), async (req, res) => {
  try {
    const { status } = req.query;
...
router.put('/events/:id/approve', authorizeRoles('admin'), async (req, res) => {
  try {
...
router.put('/events/:id/reject', authorizeRoles('admin'), async (req, res) => {
  try {
    const eventId = Number(req.params.id);
    if (!Number.isInteger(eventId) || eventId <= 0) return res.status(400).json({ error: 'Invalid event id.' });
...
router.get('/stats', authorizeRoles('admin'), async (req, res) => {
  try {
    const students = await db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'student'").get();
    const lecturers = await db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'lecturer'").get();
    const totalCourses = await db.prepare('SELECT COUNT(*) as count FROM courses').get();
    const draftCourses = await db.prepare("SELECT COUNT(*) as count FROM courses WHERE status = 'draft'").get();
    const publishedCourses = await db.prepare("SELECT COUNT(*) as count FROM courses WHERE status = 'published'").get();
    const materials = await db.prepare('SELECT COUNT(*) as count FROM materials').get();
    const pendingNews = await db.prepare("SELECT COUNT(*) as count FROM news WHERE status = 'pending'").get();
    const pendingEvents = await db.prepare("SELECT COUNT(*) as count FROM calendar_events WHERE status = 'pending'").get();
